...
mod tests {
    ...
    const ALL_PERMUTATIONS: &[&[usize]] = &[&[0, 1, 2], &[0, 2, 1], &[1, 0, 2], &[1, 2, 0], &[2, 0, 1], &[2, 1, 0]];
    ...
    /// Test if ranking three candidate produces one of the `possible_results`. All permutations
    ...
    fn assert_possible_rankings(paths: &Paths, possible_results: &[&[usize]]) {
        for initial_state in ALL_PERMUTATIONS {
            ...
            assert!(possible_results.contains(&&ranking[..]), "{:?} not in {:?}", ranking, possible_results);
        }
    }
